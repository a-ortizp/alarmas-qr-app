...
- **App móvil (asistente y organizador):** 16 pantallas, M00a–M13 (incluye M02b, vista calendario), más tres auxiliares en los mockups de alta fidelidad: M02v (inicio sin alarmas), M02h (hoja «Agregar evento») y M03b (pantallazo recibido).
- **Web (administración y consulta):** 8 pantallas, W00–W07. La web ya **no crea ni edita eventos** (eso vive en el móvil, F-M07); es de consulta, reportes, difusión y gestión de la cuenta. En los **mockups de alta fidelidad** (página `03 · Web`, 2026-09-14, ver `MOCKUPS.md` §7) son 14 marcos: W02 se integra en W01 como pestañas Todos / Creados / Escaneados, W04 y W05 son modales sobre W01 y W07 es el modal «Eliminar cuenta» sobre W06.
...
- **Conexiones del prototipo:** el bloque "Alarma conectada · Confirmar antes de auto-ajustarse" → M09 (⏩ simula la llegada del push del organizador); "Guardar cambios", "Eliminar alarma" y "‹" → M02; "Gestionar en el calendario" → M02b.
...
- Tres salidas: **"Perfecto, así queda"** (aceptar), **"Mantener mi alarma"** (conservar la previa) o **"Eliminar alarma"**.
- **Conexiones del prototipo:** M09 ya no es un punto de inicio aparte; se llega desde M06. "Perfecto, así queda" (en los mockups «Aceptar el cambio») → M10 (⏩ salto temporal a las 4:45 pm, la hora recalculada); "Mantener" → M02; el título del cambio → M06. En los mockups v1.1 «Eliminar alarma» sale del cuerpo de M09 (queda en M06) y la barra tiene «×» → M02.
...
- **Mockups web:** W00 `4072:1861`, tarjeta centrada con la marca, «Iniciar sesión» → W01 y la nota «¿Aún no tienes cuenta? Créala desde la app móvil»; variante W00 (cuenta eliminada) `4072:1878` con snackbar «Cuenta eliminada exitosamente».
...
- **Mockups web:** W01 «Mis Alarmas» `4072:26` es el hub: cuatro indicadores (6 eventos activos · 128 escaneos · 97 alarmas activas, conversión 76 % · 41 «Ya voy») sobre la tabla de eventos, más los botones «Exportar reporte» (→ modal W04) y «Descargar QR en lote» (contorno, → modal W05). Desde la web v1.1 incluye la tarjeta «Escaneos por semana» (8 semanas, total 128).
...
- **Mockups web:** integrada en W01 como pestañas Todos / Creados (`4072:49`) / Escaneados (`4072:72`), chips «Creada por mí» / «✓ Escaneada», estado «Publicado», «Ver detalle ›» → W03 y la nota «Las métricas de eventos escaneados pertenecen a su organizador».
...
- **Mockups web:** W03 «Detalle Evento» `4072:95` (Partido Sintética): miga de pan, 16 escaneos · 11 alarmas activas (5 la eliminaron) · 7 «Ya voy», tabla «Quiénes escanearon» con alias (Joale7, Mike1008, J.P, CarrosC6), estado Activa / Eliminada, buscador y «Ver todos ›».
...
- **Mockups web:** modal «Exportar reporte consolidado» sobre W01 (`4072:118`): rango Último mes / Semestre / Rango personalizado (fechas, `4072:141`), formato PDF / CSV, nota Ley 1581, «Generar y descargar»; estado Listo (`4072:164`) con «reporte-alarmasqr-ago2026.pdf generado y descargado exitosamente» y «Generar de nuevo».
...
- **Mockups web:** modal «Descargar QR en lote» sobre W01 (`4072:187`): «Seleccionar todos» (2 de 2), lista con QR y estado, bloque «Vista previa del afiche» (marca Alarmas QR + QR + nombre del evento + «Escanéalo y te avisamos»; QR mínimo 4 × 4 cm), formato PNG / PDF, «Descargar»; estado Completado (`4072:210`) con snackbar.
...
- **Mockups web:** en W01, buscador + filtros Próximos / Pasados / Borradores + pestañas Todos / Creados / Escaneados.
...
En los **mockups web** (página `03 · Web`): F-W00 → W00 · F-W01, F-W02 y F-W06 → W01 «Mis Alarmas» (indicadores + tabla con pestañas) · F-W03 → W03 · F-W04 → modal «Exportar reporte» sobre W01 · F-W05 → modal «Descargar QR en lote» sobre W01 · F-W07 → W06 · F-W08 → modal «Eliminar cuenta» sobre W06.
...
| T5 | Web: entrar, filtrar eventos y bajar reporte y afiches | W00 → W01 → W02 → W03 → W04 · W05 (mockups: W00 → W01 con pestañas → W03 → modales W04 · W05) |
