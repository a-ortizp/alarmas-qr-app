...
- **2026-09-24 · v1.12 (solo documento)** — Peso de las horas grandes. «Datos» (Spline Sans Mono) es una fuente variable y el eje `wght` de su instancia Bold se ve más delgado de lo esperado a 52 y 96 pt: al pedir un peso que la familia no tiene, Android sintetiza trazo extra sobre el Bold real en vez de limitarse a escogerlo, y eso es lo que muestran los marcos de M09 y M10. Los bloques «Sonará» (M04/M09, 52) y la hora protagonista de la alarma sonando (M10, 96 + sufijo am/pm 28, ambos al mismo peso) pasan de **Bold 700 a Black 900**; la hora en tarjeta (26) y su am/pm (12) se quedan en 700 y 500, porque a ese tamaño el Bold real sí coincide con el marco. `design-tokens.json` v1.20 (`font.scale.hora-protagonista`, `font.scale.hora-sonara`). Sin cambios en el archivo Figma del DS ni en el PDF. **Pendiente de replicar en el repo de UX**, de donde `docs/` es copia.
