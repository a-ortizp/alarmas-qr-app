...
- **Paginador de W03 fijo en 2 páginas (`[totalPaginas]="2"`):** el texto «Mostrando 1–4 de 16 asistentes · 4 por página» sugiere 4 páginas, pero el dataset real (`dataset.json`, `web.asistentes`) solo trae dos páginas de datos de asistentes (`mostrados` + `pagina2`, 8 filas en total) — limitación del tamaño del dataset, no un defecto de la pantalla. Aceptado tal cual; resolverlo requeriría más filas en `dataset.json` (fuera de alcance — `dataset.json` nunca se modifica).
