...
Filas M02b, M06, M06d, M07, M08, M09, M10, M11 y M11d (Tarea 11, Plan 4 · móvil de la Persona B) siguen la misma
metodología: exportadas con `mcp__plugin_figma_figma__get_screenshot` (archivo `4nHD4ygcnP33UH0gAhaii5`, mismo
Figma del equipo, ids de `docs/MOCKUPS.md` §5, mapeados en el orden de flujo documentado ahí: M02b = `4:2`,
M06 = `5:2`, M06d = `4329:1432`, M07 = `5:60`, M08 = `5:130`, M09 = `5:195`, M10 = `6:2`, M11 = `6:28`,
M11d = `4331:1448`) y comparadas a ojo contra las capturas reales de `capturar()`. A diferencia de las pantallas de
la Persona A, varias de estas pantallas implementan un subconjunto simplificado del marco (decidido en el brief de
la tarea del plan que las construyó, no en esta Tarea 11): el detalle exacto de cada simplificación va en la fila
correspondiente. Estas nueve capturas se regeneraron el 2026-09-24 tras los rediseños de M06, M08, M09, M10 y M11
...
| M02b | 4:2 | ok · comparación real. `M02b.png` se captura standalone en `M02bCalendarioScreenTest` (sin el `Scaffold` raíz de `NavegacionApp`), así que sigue sin la barra inferior ni el FAB del marco (viven en el Scaffold compartido con M02/M05/M11, mismo patrón que `M04-contenido.png` de la Tarea 12). El resto alinea con el marco: barra «Mis alarmas» con el selector «Lista/Mes» (píldora activa en Tinta), encabezado «Agosto 2026» con las dos flechas, fila de iniciales L M X J V S D, rejilla con el día de hoy (27) resaltado en Tinta con número blanco y guion bajo los días con alarma (criterio «color + forma»), agrupador «JUE 27 · 2 ALARMAS» y las dos tarjetas del día. La tarjeta «Reunión con el tutor» ya no muestra el chip «Creada por mí» de `dataset.json` (la lista del calendario monta `TarjetaAlarma` con `mostrarChips = false`), igual que el marco. Diferencia: la rejilla dibuja una sexta fila con el 31 (marcado, alarma «Asado del semillero») y los días grises del 1 al 6 de septiembre, mientras el marco corta el mes en el 30 |
| M06 | 5:2 | ok · comparación real. `M06.png` (montado standalone en `M06EditarAlarmaScreenTest`, `a-tutor`) ya trae todos los bloques del marco y en el mismo orden: barra «‹ Editar alarma», tarjeta de resumen (hora + título + «fecha corta · evento h:mm» + chip de origen), selector ANTICIPACIÓN (10 min/30 min/1 h/Otro, pastilla activa en Tinta), «Sumar trayecto desde mi ubicación», selector SONIDO (Sonar/Vibrar/Silencio), «Respetar "No molestar"», «Posponer · 10 min ›», encabezado CAMBIOS DEL ORGANIZADOR con «Confirmar antes de auto-ajustarse», campo NOTAS, la fila gris «Gestionar en el calendario ›», «Guardar cambios» (amarillo, único amarillo de la pantalla) y «Eliminar alarma» (enlace Coral Texto). Diferencias: la captura usa `a-tutor` («Reunión con el tutor», 7:30 am, «jue 27 · evento 8:00 am», chip «Creada por mí») mientras el marco ilustra `a-entrega` («Entrega de proyecto UX», 3:15 pm, chip «✓ Escaneada») — ambos son datos de `dataset.json`; por lo mismo NOTAS sale vacío (solo su etiqueta), porque `a-tutor` no trae notas, mientras el marco muestra «Llevar maqueta impresa» (verificado aparte con `a-entrega` en la misma prueba). «Guardar cambios» y «Eliminar alarma» van al final de la columna desplazable, justo bajo «Gestionar en el calendario», en vez de anclados al borde inferior como en el marco |
| M06d | 4329:1432 | ok · pixel-perfect en el contenido del diálogo: título «¿Eliminar alarma?», cuerpo con el mensaje interpolado («Dejarás de recibir el aviso de "Gimnasio" (jue 27 · 9:30 am)...»), «Conservar» amarillo primario y «Eliminar» contorno Coral Texto, radio 20 e igual jerarquía que el marco (acción segura prominente). El velo Tinta 55 % y la pantalla M06 completa detrás también coinciden con el marco. Diferencia: la prueba abre el diálogo sobre `a-gimnasio` («Gimnasio», 9:00 am), así que el cuerpo interpola «"Gimnasio" (jue 27 · 9:30 am)» y ocupa 3 líneas, mientras el marco ilustra `a-entrega` («Entrega de proyecto UX», dom 30 · 4:00 pm) — dato de `dataset.json`, no un defecto. El `Dialog` en sí es el mismo componente `DialogoConfirmacion` ya verificado pixel-perfect en M04d |
| M07 | 5:60 | ok · comparación real. Alcanzable solo por flujo (`FlujosPersonaBTest`, T3: M02 → M02h → M07), capturado ahí con `regla.capturar("M07")` tras montar la pantalla vía `entradasApp`. La pantalla alinea con el marco: barra «‹ Nuevo evento», TÍTULO, FECHA y HORA lado a lado, LUGAR (OPCIONAL) y DESCRIPCIÓN (OPCIONAL) vacíos con solo su etiqueta, los cuatro chips de ANTICIPACIÓN (10 min/30 min/1 h/Otro, «30 min» activo en Tinta), la tarjeta gris con el ícono QR y la nota «El código QR del evento se genera automáticamente al guardar, listo para compartir.», y «Guardar y crear QR» amarillo anclado abajo. Diferencias: FECHA y HORA son de solo lectura con el valor por defecto «Mañana» / «10:00 am» (MISO no tiene selector de fecha construido, documentado en el KDoc de `M07CrearEventoScreen`) en vez de «Lun 31 ago» / «12:30 pm» del marco; el campo TÍTULO se dibuja en reposo, mientras el marco lo muestra enfocado con borde Tinta |
| M08 | 5:130 | ok · comparación real. Alcanzable solo por flujo (mismo T3 de `FlujosPersonaBTest`, tras «Guardar y crear QR»), capturado con `regla.capturar("M08")`. Todos los bloques del marco están y en el mismo orden: barra «‹ Compartir evento», tarjeta blanca con el título «Asado familiar», la fecha/hora del evento, el QR (tag `qr`), «Escanéalo y te avisamos» y su nota; debajo «Tu alarma quedó programada · sonará…», el chip amarillo «Aún sin escaneos · recién creado», la fila de iconos WhatsApp/Correo/Más, «Descargar PNG» y «Descargar PDF» en contorno lado a lado, «Compartir por WhatsApp» amarillo y «Copiar enlace del evento» como enlace azul subrayado. Diferencias: la fecha/hora dicen «Vie 28 de agosto · 10:00 am» y «sonará 9:30 am» frente a «Lun 31 de agosto · 12:30 pm» / «12:00 pm» del marco, porque el evento se crea en M07 con su fecha y hora por defecto (ver fila M07) y 30 min de anticipación; el patrón de módulos del QR es el que dibuja el componente real, distinto del QR ilustrativo del marco |
| M09 | 5:195 | ok · comparación real. La pantalla ya es Blanco Papel con la banda de textura de módulos QR arriba (como el marco y MOCKUPS.md, fila «M04/M09»), con la cabecera «Cambio en tu evento ✕», el chip «Notificación push · hace 2 min», el título H2 «"Entrega de proyecto UX" cambió de hora», la línea «Cambio hecho por MISO · UniAndes» con el chip «✓ verificado» en contorno verde, la tarjeta ANTES/AHORA con el valor antiguo tachado, el bloque Amarillo Suave «SI ACEPTAS, SONARÁ» con «4:45 pm» en Spline Sans Mono y «30 min de margen + trayecto · antes sonaba 3:15 pm», «Aceptar cambio» amarillo y «Mantener alarma» en contorno Tinta. Diferencia: la fila AHORA dice «dom 30 · 5:30 pm · Aula SD-703, Universidad» y se parte en dos líneas, porque el lugar sale de `dataset.json` (`a-entrega.lugar`) y la fecha corta va en minúscula, mientras el marco lo escribe «Dom 30 · 5:30 pm · misma aula SD-703» en una línea |
| M10 | 6:2 | ok · comparación real. Coincide con el marco en todo el bloque: banda de textura arriba sobre fondo Tinta, la ilustración de esfera de reloj con ondas Coral, el chip «● Alarma de evento» en contorno Coral, la hora protagonista en Amarillo Energía (Spline Sans Mono con el sufijo en la misma línea), el título del evento, la línea «Hoy · evento … · Aula SD-703», la tarjeta de tráfico con fondo Amarillo Suave («Sal en 12 min · con el tráfico actual llegas 5:22 pm» + «Ver ruta ›»), «Ya voy» (blanco, excepción documentada «sobre Tinta el primario es blanco») y «Posponer 10 min» en contorno, ambas de 56, y la nota final «En reunión o clase solo vibra · si no confirmas, re-aviso en 5 min». Diferencias: la hora capturada es «3:15 pm» y el evento «4:00 pm», los valores base de `a-entrega` en `dataset.json` (la prueba no pasa por M09), frente a «4:45 pm» / «5:30 pm» post-cambio que ilustra el marco — mismo criterio ya documentado en M02/M09; «Ver ruta ›» cae en una segunda línea dentro de la tarjeta en vez de quedar al final de la misma línea |
| M11 | 6:28 | ok · comparación real. `M11.png` se captura standalone en `M11AjustesScreenTest` (mismo patrón sin Scaffold que M02b: sin barra inferior). Las cuatro secciones del marco están completas y en orden: «ALARMAS» (Anticipación por defecto · 30 min ›, Sonido predeterminado ›, Posponer predeterminado · 10 min ›, «Respetar "No molestar" (vibrar)» y «Confirmar reajustes del organizador» con interruptor), «PERMISOS DEL SISTEMA» con las píldoras «✓ activo»/«✓ activo»/«Revisar» (contorno verde/verde/coral, criterio «color + forma») y la nota «Sin este permiso, Android puede silenciar la alarma en segundo plano.», «CALENDARIOS» con «Google Calendar · Conectado ›», «CUENTA Y DATOS» con «Gestionar mis datos (Ley 1581) ›» y «Cerrar sesión ›», y el pie «Alarmas QR · versión 1.0 · prototipo». Diferencias: «Sonido predeterminado» muestra «Sonar» (valor de `dataset.json`, `usuario.ajustes.sonidoPorDefecto`) donde el marco escribe «Clásico»; la captura es standalone en `M11AjustesScreenTest`, así que no incluye la barra inferior del marco (vive en el `Scaffold` compartido, mismo patrón que M02b) |
| M11d | 4331:1448 | ok · pixel-perfect en el contenido del diálogo: título «¿Cerrar sesión?», cuerpo «Tus alarmas quedan guardadas en tu cuenta. Para volver a verlas y recibir los cambios del organizador tendrás que iniciar sesión de nuevo.» idéntico al marco, «Cancelar» amarillo primario (acción segura) y «Cerrar sesión» en contorno Tinta (no Coral, correcto: cerrar sesión no es destructivo) — coincide con la regla de `DialogoConfirmacion` de CLAUDE.md. El velo Tinta 55 % y la pantalla M11 completa detrás también alinean con el marco; las únicas diferencias son las de la fila M11 (el «Sonar» del fondo y la barra inferior ausente por ser una captura standalone) |
...
Parejas a 1280×820: `<nombre>-figma.png` (exportada con `get_screenshot` del MCP de Figma, `maxDimension` 1280) y `<nombre>.png`. Esta última se captura con Playwright MCP a 1280×820 (`page.setViewportSize`) sobre `ng serve` — Chrome sin interfaz (`--headless=new --window-size=1280,820`) deja un viewport real de 1280×733, así que no se usa (ver nota de captura más abajo y `docs/superpowers/plans/2026-09-21-plan3-web-persona-a.md`, Tarea 8). Diferencias aceptadas por decisión del plan: campos de 48 con etiqueta 12 y placeholder Gris Medio (D3), botones de 14.5 (D5), snackbar centrado en la ventana (D7) y «¿Olvidaste tu contraseña?» también en los estados con snackbar (D8).
...
- **`aq-chip` en píldora completa en vez de `--radius-chip-web`:** usa `--radius-pildora` (999px) en lugar del token `--radius-chip-web` (8px) que pide el DS §7 y las restricciones globales del propio plan — así quedó especificado literalmente en el brief de la Tarea 2 para el componente; es una inconsistencia de redacción del plan, no un defecto de implementación. Aceptado tal cual.
- **Paginador de W03 con 4 páginas, 2 inventadas en el componente:** el dataset real (`dataset.json`, `web.asistentes`) solo trae dos páginas de asistentes (`mostrados` + `pagina2`, 8 filas). Para que el paginador llegue a las 4 páginas que el texto «Mostrando 1–16 de 16 asistentes · 4 por página» implica, `w03-detalle-evento.component.ts` define `ASISTENTES_INVENTADOS` (8 filas más, alias y fechas ficticios) para las páginas 3 y 4 — a pedido explícito del usuario el 2026-09-22, nunca tocando `dataset.json`. La columna «Confirmó "Ya voy"» siempre muestra «—» en las cuatro páginas (así la dibuja el mockup; no se deriva de `confirmoYaVoy`/`yaVoy`).
