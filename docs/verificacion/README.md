...
| M02b | 4:2 | ok · comparación real contra Figma (Tarea 11, acceso MCP disponible). `M02b.png` se captura standalone en `M02bCalendarioScreenTest` (sin el `Scaffold` raíz de `NavegacionApp`): no incluye la barra inferior ni el FAB del marco (viven en el Scaffold compartido con M02/M05/M11, mismo patrón que `M04-contenido.png` de la Tarea 12); su `BarraSuperior("Calendario")` tampoco trae el selector de pestaña «Lista/Mes» ni el encabezado de mes «Agosto 2026» con flechas ni la fila de iniciales de día (L M X J V S D) del marco — así quedó escrito literalmente en el brief de la Tarea 3 del plan (navegación por pestañas inferiores en vez del selector superior del mockup), decisión del plan y no un defecto de esta tarea. La rejilla del mes sí marca los días con alarma con un punto bajo el número (equivalente en forma al guion del marco) y el día de hoy (27) resaltado en Tinta con número blanco, igual criterio «color + forma» que el marco. La tarjeta «Reunión con el tutor» trae el chip «Creada por mí» que el marco no dibuja en esa fila (viene de `dataset.json`, fuente única de datos) |
| M06 | 5:2 | ok · comparación real. `M06.png` (montado standalone en `M06EditarAlarmaScreenTest`, `a-tutor`) reproduce con fidelidad los bloques que sí implementa: título, selector ANTICIPACIÓN (chips 10 min/30 min/1 h/Otro · N min, pastilla activa en Tinta), selector SONIDO, «Guardar cambios» (amarillo, único amarillo de la pantalla) y «Eliminar alarma» (enlace Coral Texto). El brief de la Tarea 4 del plan (código de ejemplo del propio brief) no incluye la tarjeta de resumen del evento (hora + título + chip «✓ Escaneada»), el interruptor «Sumar trayecto desde mi ubicación», el interruptor «Respetar "No molestar"», la fila «Posponer 10 min ›» ni el campo «NOTAS» que sí trae el marco 5:2 — es una simplificación decidida en el propio brief de esa tarea (ya revisado y fusionado antes de esta Tarea 11), no algo que esta verificación deba corregir. La fila «Alarma conectada · Confirmar antes de auto-ajustarse» combina en una sola fila lo que el marco separa en el encabezado «CAMBIOS DEL ORGANIZADOR» + fila «Confirmar antes de auto-ajustarse»; «Gestionar en el calendario» se dibuja como enlace azul subrayado sobre «Guardar cambios» en vez de la fila gris con chevron que trae el marco debajo de SONIDO |
| M06d | 4329:1432 | ok · pixel-perfect en el contenido del diálogo: título «¿Eliminar alarma?», cuerpo con el mensaje interpolado («Dejarás de recibir el aviso de "Gimnasio" (jue 27 · 9:30 am)...»), «Conservar» amarillo primario y «Eliminar» contorno Coral Texto, radio 20 e igual jerarquía que el marco (acción segura prominente). El diálogo aparece más arriba en `M06d.png` que en el marco porque el contenido de M06 detrás es más corto que el del marco (ver fila M06 arriba); el `Dialog` en sí es el mismo componente `DialogoConfirmacion` ya verificado pixel-perfect en M04d |
| M07 | 5:60 | ok · comparación real. Alcanzable solo por flujo (`FlujosPersonaBTest`, T3: M02 → M02h → M07), capturado ahí con `regla.capturar("M07")` tras montar la pantalla vía `entradasApp`. El formulario (TÍTULO, LUGAR, DESCRIPCIÓN vacíos con solo su etiqueta, chips de ANTICIPACIÓN, «Guardar y crear QR» amarillo) coincide en estructura y campos vacíos-solo-etiqueta con el marco. Diferencias: el título de la barra dice «Crear evento» (marco: «Nuevo evento»); FECHA y HORA son un solo campo «Fecha y hora· Mañana · 10:00 am» de solo lectura en vez de dos campos lado a lado (marco: «Lun 31 ago» / «12:30 pm»); solo 3 chips de anticipación (10 min/30 min/1 h, marco trae también «Otro»); falta la tarjeta gris con el ícono QR y la nota «El código QR del evento se genera automáticamente al guardar, listo para compartir.». Estas simplificaciones vienen del alcance de la tarea que implementó M07 (anterior a esta Tarea 11); no se modifican aquí |
| M08 | 5:130 | ok · comparación real. Alcanzable solo por flujo (mismo T3 de `FlujosPersonaBTest`, tras «Guardar y crear QR»), capturado con `regla.capturar("M08")`. El QR (tag `qr`), el título del evento y los tres botones/acciones (Compartir por WhatsApp, Descargar, Copiar enlace) están presentes y disparan sus callbacks reales. Diferencias frente al marco: el título de la barra dice «QR del evento» (marco: «Compartir evento»); sin la tarjeta blanca con sombra que envuelve título+QR ni la fecha/hora bajo el título ni «Escanéalo y te avisamos» + su nota; sin la línea «Tu alarma quedó programada · sonará…»; sin la fila de iconos WhatsApp/Correo/Más; un solo botón «Descargar como imagen» en vez de «Descargar PNG»/«Descargar PDF»; «Copiar enlace» es un botón de contorno en vez de un enlace azul subrayado. Curiosidad de captura: se ve un pequeño subrayado bajo la «Q» de «QR del evento» en `M08.png` — es el anillo de foco de accesibilidad que Robolectric deja sobre el primer elemento enfocable de la ventana nueva (la barra «‹»/título), no reproducible en un dispositivo real ni parte del diseño |
| M09 | 5:195 | ok · comparación real. Diferencia notable: `M09CambioEventoScreen` usa fondo Tinta pleno con `sobreTinta = true` en textos/botones (código literal del brief de la Tarea 7 del plan), mientras el marco 5:195 usa Blanco Papel + banda de textura (MOCKUPS.md, fila «M04/M09»); es una decisión de ese brief anterior a esta tarea, no algo que corregir aquí. También faltan el chip «Notificación push · hace 2 min», la tabla «ANTES/AHORA» con tachado y el chip «✓ verificado» que trae el marco. Lo que sí coincide con precisión: el bloque «SI ACEPTAS, SONARÁ» con la hora «4:45 pm» en Amarillo Energía (Spline Sans Mono, mismo tratamiento tipográfico que el marco), «30 min de margen + 15 min de trayecto», «Antes sonaba 3:15 pm», «Aceptar cambio» en blanco (excepción documentada «sobre Tinta el primario es blanco») y «Mantener alarma» en contorno blanco |
| M10 | 6:2 | ok · comparación real. Estructura general coincide con el marco (banda de textura arriba, fondo Tinta, hora protagonista «3:15 pm» en Amarillo Energía Spline Sans Mono, dos acciones de 56 pt ancladas abajo). Diferencias: el ícono superior es `Destello` (mismo componente reutilizado en M01/M02v/M08) en vez de la ilustración «esfera de reloj con ondas coral» del marco (no existe ese asset en la librería de iconos del proyecto); sin el chip «● Alarma de evento»; «Ver ruta ›» se dibuja como enlace bajo «Sal en 12 min · tráfico moderado» en vez de vivir dentro de un destacado con fondo Amarillo Suave; sin la nota final «En reunión o clase solo vibra · si no confirmas, re-aviso en 5 min». El botón primario sí dice «Ya voy · ver ruta», igual que el marco. La hora capturada (3:15 pm) es el valor base de `a-entrega` en `dataset.json` (la prueba no pasa por M09), distinto de la hora post-cambio (4:45 pm) que ilustra el marco — mismo criterio ya documentado en M02/M09 de mostrar el valor real del dataset en el estado capturado |
| M11 | 6:28 | ok · comparación real. `M11.png` se captura standalone en `M11AjustesScreenTest` (mismo patrón sin Scaffold que M02b: sin barra inferior). Estructura de secciones simplificada frente al marco: «NOTIFICACIONES» (No molestar, Confirmar antes de auto-ajustar) en vez de «ALARMAS» con anticipación/sonido/posponer predeterminados; «PERMISOS» con «Concedido»/«Concedido»/«Falta» en texto de color plano (verde/verde/coral) en vez de las píldoras «✓ activo»/«✓ activo»/«Revisar» del marco — mismo criterio «color + forma» (el texto mismo cambia, no solo el color) aunque sin el contorno de píldora; «CALENDARIOS» reducido a «Vinculados: google» en vez de la fila «Google Calendar · Conectado ›»; «CUENTA» sin la fila «Gestionar mis datos (Ley 1581)» ni el pie «Alarmas QR · versión 1.0 · prototipo». Alcance decidido en el brief de la Tarea 9 del plan, anterior a esta tarea |
| M11d | 4331:1448 | ok · pixel-perfect en el contenido del diálogo: título «¿Cerrar sesión?», cuerpo «Tus alarmas quedan guardadas en tu cuenta. Para volver a verlas y recibir los cambios del organizador tendrás que iniciar sesión de nuevo.» idéntico al marco, «Cancelar» amarillo primario (acción segura) y «Cerrar sesión» en contorno Tinta (no Coral, correcto: cerrar sesión no es destructivo) — coincide con la regla de `DialogoConfirmacion` de CLAUDE.md. El diálogo queda más arriba en `M11d.png` que en el marco por el mismo motivo que M06d (menos contenido detrás, ver fila M11) |
