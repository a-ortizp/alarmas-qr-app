...
Fuente de verdad: `packages/tokens/design-tokens.json` v1.13 (2026-09-21: medidas web de la Persona A, Plan 3, cortes de ancho `breakpoint` y, en la rama `feature/web-responsive` pendiente del visto bueno del profesor, web responsive hasta 360; derivado de `docs/DESIGN_SYSTEM.md` v1.10; mockups móviles v1.7 del 2026-09-19: `chip-control` 32 para chips que se tocan, `icono-visor` 48, espaciador flexible en barras superiores). Nunca escribir colores, tamaños ni radios a mano: usar los tokens.
...
- **Alturas**: botones 52 en móvil (toque; 48 hasta el 2026-09-17) y 44 en web (puntero), 48 en web bajo `--breakpoint-web-telefono` (toque, tokens `breakpoint-telefono`); acciones de la alarma sonando 56; campos 48; área táctil mínima 48.
...
- **Responsive web (v1.13):** toda página nueva debe verse sin scroll horizontal de 360 a 1280+. Medidas por ancho: solo tokens (si falta uno, se agrega a `breakpoint-telefono` y a `:root[data-ancho~="telefono"]` de los dos `tokens.css`). Composición por ancho: señales de `CortesService` (`apilarColumnas`, `cajon`, `telefono`) con un atributo `data-*` y su regla CSS, como W06 y el modal «Eliminar cuenta». Un contenedor de grilla centrado usa `grid-template-columns: minmax(0, 1fr)` para que un ancho de Figma con `max-width: 100%` pueda encogerse; tablas anchas dentro de `overflow-x: auto`. Prueba en `web-responsive.spec.ts`.
...
- En la web (v1.5) Reportes (`/reportes`) y Descargar QR (`/qr`) son páginas con miga «‹ Mis alarmas», título y una tarjeta de formulario de 600 px con una tarjeta lateral; el único modal es «Eliminar cuenta» (ruta propia sobre `/perfil`, velo Tinta al 45 %) y «Cerrar Sesión» abre el diálogo de confirmación web (velo 55 %, 420 px, «Cancelar» primario / «Cerrar sesión» contorno). La barra lateral lleva un icono por ítem y se colapsa a 64 px, también sola con una ventana más angosta que `--breakpoint-web-colapsar-barra` (1200). Las páginas de dos columnas se apilan con una ventana más angosta que `--breakpoint-web-apilar-columnas` (1100). **Web responsive (v1.13, rama `feature/web-responsive`):** con una ventana más angosta que `--breakpoint-web-cajon` (900) la barra lateral sale del grid y se abre como cajón de 208 desde el botón ☰ «Abrir menú» de la barra superior, sobre velo Tinta 55 % (cierran Escape, el velo, «Cerrar menú» o elegir un ítem; no es ruta, es estado del layout); con una más angosta que `--breakpoint-web-telefono` (600) `<html>` lleva `data-ancho="telefono"` y `tokens.css` reemplaza medidas de puntero por las de toque (botones y segmentado 48, márgenes 16); ancho mínimo soportado 360, sin scroll horizontal de página. Todos los cortes se leen con `CortesService` (nunca `@media` con px a mano); el marco de referencia 1280×820 queda por encima de todos. Detalle en el README («Web responsive»). En móvil, las hojas M02h y M04 son claves del back stack dibujadas como hoja inferior por `HojaInferiorSceneStrategy` sobre velo Tinta al 55 %; los diálogos de confirmación (M04d, M06d, M11d) no son rutas: son estado del componente padre sobre el mismo velo.
